...
- **Clustering only the regression coefficients:** In the main BSTC-DP model,
  ...
  $\phi_s = (\beta_s, \xi_s)$. An alternative specification applies the
  Dirichlet Process only to $\beta_s$, while $\xi$ is estimated as a single
  global temporal persistence parameter. This sensitivity analysis assesses
  whether the large number of inferred clusters is primarily induced by
  municipality-specific temporal persistence.

- **Dispersion of the DP base distribution:** The baseline DP uses
  $\beta_k \sim \mathcal{N}(0, I)$. A more concentrated base distribution,
  $\beta_k \sim \mathcal{N}(0, 0.25 I)$, was also considered, reducing the
  prior standard deviation of each cluster-specific coefficient from 1 to 0.5.
  This tests whether allowing highly dispersed cluster-specific regression
  profiles contributes to excessive partition fragmentation.

- **Dirichlet Process concentration prior:** Because the expected number of
  clusters under a Dirichlet Process depends on both the concentration
  parameter $\alpha_{DP}$ and the number of areal units, the prior on
  $\alpha_{DP}$ was recalibrated for the municipal setting. In addition to the
  original $\mathrm{Gamma}(3,2)$ specification, more parsimonious priors were
  considered, including calibrations targeting approximately the same prior
  expected number of clusters as in the provincial application and a stronger
  sensitivity targeting $E[K_S] \approx 4$.
